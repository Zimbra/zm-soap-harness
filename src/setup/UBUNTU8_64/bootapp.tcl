...
#set up staf tarball
expect {
    -ex # {send "wget --no-proxy -O /var/tmp/staf.tgz http://zqa-tms.eng.vmware.com/setup/common/staf-64.tar.gz;rm -r -f /usr/local/staf;tar -C /usr/local -xvf /var/tmp/staf.tgz\r";}
}
...
expect {
    -ex # {send "wget --no-proxy -O /usr/local/staf/bin/STAF.cfg http://zqa-tms.eng.vmware.com/setup/common/STAF.cfg\r"}
}

expect {
    -ex # {send "wget --no-proxy -O /var/tmp/profile.txt http://zqa-tms.eng.vmware.com/setup/common/profile.txt; cat /var/tmp/profile.txt >> /etc/profile; source /var/tmp/profile.txt\r";}
}

#setup staf rc.local file
expect {
    -ex # {send "wget --no-proxy -O /etc/rc.local http://zqa-tms.eng.vmware.com/setup/main/UBUNTU8/bootapp.sh\r"}
}
...
expect {
    -ex # {send "wget --no-proxy -O /var/tmp/runme.pl http://zqa-tms.eng.vmware.com/setup/main/UBUNTU8_64/bootapp.pl;chmod ugo+x /var/tmp/runme.pl;/var/tmp/runme.pl\r"}
}
