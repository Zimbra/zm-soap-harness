...
namespace Tests.MailClient.Mail
{
    ...
    public class SmartForwardMail : Tests.BaseTestFixture
    {
        ...
        [Test, Description("Forward plain text email using SmartForward command"),
        Property("TestSteps", "1. Send email from account1 to EAS user, 2. Forward this mail(Note: Smart Forward) to Account2, 3. Verify the forwarded email is in Sent folder, 4. Verify the message is received by the destination, 5. Verify that forwarded message content is correct i.e. has forwarded + original content")]
        [Category("Functional")]
        [Category("Mail")]
        [Category("L2")]
        public void SmartForwardMail01()
        {
...
<Sync xmlns='AirSync' xmlns:AirSyncBase='AirSyncBase' >
    <Collections>
        <Collection>
            ...
            String itemId = Add.SelectSingleNode("//AirSync:ServerId", ZAssert.NamespaceManager).InnerText;
            ...
            #region Create Mime for SmartForward

            //Notice, we are not sending content/body of source email being forwarded
            String mime =
@"Subject: " + "Fwd: " + subject + @"
...
";
            ...
            ZSmartForwardRequest request = new ZSmartForwardRequest(TestAccount);
            request.DestinationPayloadText = mime;
            request.ItemId = itemId;
            request.CollectionId = folderid;

            ZResponse response = TestClient.sendRequest(request);
            ZAssert.IsNotNull(response, "Verify the SmartForward response was received");
            ...
            ZAssert.AreEqual(soapContent, data, "Verify that forwarded message content is correct i.e. has forwarded + original content");
        ...
        [Test, Description("Forward html text email using SmartForward command"),
        Property("TestSteps", "1. Send email from account1 to EAS user, 2. Forward this mail(Note: Smart Forward) to Account2, 3. Verify the forwarded email is in Sent folder, 4. Verify the message is received by the destination, 5. Verify that forwarded message content is correct i.e. has forwarded + original content")]
        [Category("Functional")]
        [Category("Mail")]
        [Category("L2")]
        public void SmartForwardMail02()
        {
            ...
            String forwardContentHtmlEAS = "<html><body>Forwarded the <strong>received</strong> email<br><br>----- Forwarded message -----<br><br></body></html>";
            String forwardContentServer = "<html><body><div>Forwarded the <strong>received</strong> email<br><br>----- Forwarded message -----<br><br></div><br><div>This message has html <strong>bold</strong> text</div></body></html>";
            String sentMessageId = "<" + HarnessProperties.getUniqueString() + "@" + TestAccount.DomainName + ">";
...
MIME-Version: 1.0";
            ...
            String mime = MimeBuilder.getHtmlMsgMime(mimeText, plainContent, forwardContentHtmlEAS);
...
<Sync xmlns='AirSync' xmlns:AirSyncBase='AirSyncBase' >
    <Collections>
        <Collection>
            ...
            String data = Add.SelectSingleNode("//AirSyncBase:Data", ZAssert.NamespaceManager).InnerText;
            ZAssert.AreEqual(data, forwardContentServer, "Verify that content is correct in sent folder");
            ...
            ZAssert.AreEqual(soapContent, forwardContentServer, "Verify that content matches in sent and received message");
