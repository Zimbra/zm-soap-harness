...
namespace Soap
{
    ...
    public class SoapTest : SoapClient
    {
        ...
        public SoapTest(EndpointReference destination)
            : base(destination)
        {
        }
        ...
        public XmlNode select(XmlNode context, string path, string attr, string match, out string set, int occurences)
        {
            ...
            try
            {
                ...
                if (path != null)
                {
                    if (context == null)
                    {
                        ZAssert.AreEqual(occurences, 0, "path '" + path + "' did not matchany elements (element not present)");
                    }
                }
                ...
                if (path != null)
                {
                    if (occurences == 0)
                    {
                        ...
                        {
                            ZAssert.AreEqual(0, nodes.Count, "Check that " + path + " doesn't finds any nodes");
                        }
                    }
                    else
                    {
                        ZAssert.Greater(nodes.Count, 0, "Check that " + path + " finds nodes");
                    }
                }
                ...
                if (match != null)
                {
                    // TODO: need to handle occurences=0
                    if (occurences == 0)
                    {
                        ZAssert.IsTrue(!matchFound, "Verify that the element value (" + match + ") doesn't exists in the response");
                    }
                    else
                    {
                ...
                if (attr != null && !attrFound) //this will be used to check if specified attribute in selectsoap is not present in the context
                {
                    // TODO: need to handle occurences=0
                    if (occurences == 0)
                    {
                        ZAssert.IsTrue(!attrFound, "Verify that the attribute (" + attr + ") doesn't exists in the response");
                    }
                    else
                    {
        ...
        public string SetDestination(string server, XmlNode request)
        {
            ...
            if (nameSpace.Equals("urn:zimbraAdmin"))
            {
                ...
                try
                {
                    mode = GlobalProperties.getProperty("admin.mode");
                    port = GlobalProperties.getProperty("admin.port");
                    path = GlobalProperties.getProperty("admin.path");
            ...
            if (nameSpace.Equals("urn:zimbraAccount") || nameSpace.Equals("urn:zimbraMail") || nameSpace.Equals("urn:zimbraIM"))
            {
                ...
                try
                {
                    mode = GlobalProperties.getProperty("soapservice.mode");
                    port = GlobalProperties.getProperty("soapservice.port");
                    path = GlobalProperties.getProperty("soapservice.path");
