...
namespace PstDataTests
{
    ...
    public class OutlookProcess : IProcess
    {
        ...
        protected string PstProcessName = "ZCSPSTImportWizard";
        ...
        public bool KillPstApplication()
        {
            log.Debug("KillPSTApplicationProcess ...");

            # region Kill PST process, since it won't exit

            Process[] killingPstProcesses = Process.GetProcessesByName(PstProcessName);
            if (killingPstProcesses.Length > 0)
            {
                foreach (Process pr in killingPstProcesses)
                {
                    pr.Kill();
                    pr.WaitForExit(3000);
                }

                log.Warn("KillPstApplicationProcess ... " + PstProcessName + " had to be killed");
            ...
            log.Debug("KillPstApplicationProcess ... already stopped.");
            return (false);
        ...
        public bool IsPstApplicationRunning()
        {
            Process[] list = Process.GetProcessesByName(PstProcessName);
            log.Debug("IsPstApplicationRunning ... " + (list.Length > 0));
            return (list.Length > 0);
