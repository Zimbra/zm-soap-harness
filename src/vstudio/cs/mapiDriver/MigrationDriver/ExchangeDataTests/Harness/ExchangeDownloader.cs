...
namespace ExchangeDataTests
{
    public class ExchangeDownloader
    {
        ...
        private Uri GetDownloadURI(string AdminUser, string AdminPwd)
        {
            ...
            // scrub the downloads page for Exchange
            ...
            try
            {
                ...
                webRequest.Credentials = new NetworkCredential(AdminUser, AdminPwd);
            ...
            while ((line = streamReader.ReadLine()) != null)
            {
                ...
                if (line.Contains("ZimbraMigration"))
                {
                    ...
                    if (m.Success)
                    {
                        ...
                        {
                            // Value contains the "downloads" portion ... i.e. "/downloads/ZCSExchangeMigrationWizard-7.1.1.82.zip"
                            // 
                    ...
                    if (!m.Success)
                    {
                        throw new HarnessException("DownloadServerExchangeExe could not find ZCSExchangeMigrationWizard in " + myUriBuilder.Uri + " " + line);
                    }
            ...
            throw new HarnessException("DownloadServerExchangeExe could not find ZCSExchangeMigrationWizard!");
        ...
        private String GetMigrationToolFromWeb()
        {
            ...
            //Create a temporary account To access just index.html page and download migration tool.
        ...
        public string GetMigrationTool()
        {
            ...
            // Determine if a value is specified in migration.properties. IF value is specified in migration.prperties, 
            // then make sure the correct migration tool is present in the in the specified path otherwise, manually copy it to the specified path.
            //this solution is implemented when download/index.html was broken and there was not migration tool available.
            // When the fix is implemented, to download the tool from index.html, set MigrationToolPath to "" in migration.properties and leave this code as it is (for future use).
        ...
        private void DownloadZipFile(string MigrationZipFilePath, Uri uri, string UserName, string Password)
        {
            ...
            # region Download the Exchange Exe to the ZimbraQA folder
            try
            {
            ...
            {
                throw new HarnessException("DownloadServerExchangeExe tried to download " + uri + " to " + MigrationZipFilePath, e);
            }
        ...
        private void DecompressMigrationZip(string MigrationZipFilePath)
        {
            ...
            //foreach (ZipEntry entry in migrationZip.EntriesSorted)
            //{
            //    if (entry.FileName.Contains("ZimbraMigration"))
            //    {
            //        MigrationExecutable = entry.FileName;
            //        log.Info("The executable exchange file name is: " + MigrationExecutable);
            //        runMigrationFile = Path.GetDirectoryName(MigrationZipFilePath) + "\\" + MigrationExecutable;
