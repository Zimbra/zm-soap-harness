...
namespace NewPSTDataTests
{
    ...
    public class OutlookProcess : IProcess
    {
        ...
        protected string MigrationProcess1Name = "ZimbraMigration";
        protected string MigrationProcess2Name = "ZimbraMigrationConsole";
        ...
        public void KillMigrationApplication()
        {
            log.Debug("KillMigrationApplication ...");

            # region Kill Migration process, since it won't exit

            Process[] killingMigrationProcesses = Process.GetProcessesByName(MigrationProcess1Name);
            if (killingMigrationProcesses.Length > 0)
            {
                foreach (Process pr in killingMigrationProcesses)
                {
                    pr.Kill();
                    pr.WaitForExit(3000);
                }

                log.Warn("killingMigrationProcesses ... " + MigrationProcess1Name + " had to be killed");
                System.Threading.Thread.Sleep(3000);
            }

            killingMigrationProcesses = Process.GetProcessesByName(MigrationProcess2Name);
            if (killingMigrationProcesses.Length > 0)
            {
                foreach (Process pr in killingMigrationProcesses)
                {
                ...
                log.Warn("killingMigrationProcesses ... " + MigrationProcess2Name + " had to be killed");
                System.Threading.Thread.Sleep(3000);
            }

            #endregion
            log.Debug("killingMigrationProcesses ... already stopped.");
        }
        ...
        public bool IsMigrationApplicationRunning()
        {
            Process[] list1 = Process.GetProcessesByName(MigrationProcess1Name);
            Process[] list2 = Process.GetProcessesByName(MigrationProcess2Name);

            if (list1.Length > 0 || list2.Length > 0)
            {
                log.Debug("IsMigrationApplicationRunning ... " + "true");
                return true;
            }
            else
                return false;
