...
public class ResultsCore {
	...
	/**
	 * Build a 'status' string while processing.  This string is returned
	 * by getStatus() ... i.e. used as the STAF result
	 */
	...
	private List<ReportItem> correlateData(List<TestCaseResult> results, Map<String, BugState> bugStatus, Map<String, List<String>> bugTestcase, Map<String, String> bugContact) {
			...
			if ( result.mTestCaseResult == TestCaseResult.Status.PASSED ) {
				...
				for (String bugID : bugTestcase.get(result.mTestCaseName)) {
					...
					if ( state == BugState.RESOLVED || state == BugState.VERIFIED || state == BugState.CLOSED ) {
						...
						item.mBugOwner = (bugContact.containsKey(bugID) ? bugContact.get(bugID) : ReportItem.DefaultBugOwner);
				...
				if ( !bugTestcase.containsKey(result.mTestCaseName) ) {
					...
					for (String bugID : bugTestcase.get(result.mTestCaseName)) {
						...
						BugState state = bugStatus.get(bugID);
						if ( state == BugState.NEW || state == BugState.ASSIGNED || state == BugState.REOPENED || state == BugState.IN_PROGRESS || state == BugState.UNCONFIRMED ) {
							item.mBugID = bugID;
							item.mBugStatus = state;
							item.mNeedsFollowUp = false;
							item.mBugOwner = (bugContact.containsKey(bugID) ? bugContact.get(bugID) : ReportItem.DefaultBugOwner);
...
//		private static final int EqualTo = 0;
	...
	private void writeTestSummary(File root, List<TestCaseResult> results) throws IOException {
		...
		//Currently what should Errors represent is not known. Hence setting it to zero. Once we know exactly what Error is, need to modify code.
		//int errors = results.size() - TestCaseResult.frequency(results, TestCaseResult.Status.PASSED) - TestCaseResult.frequency(results, TestCaseResult.Status.FAILED);
	 ...
	 */
	public void execute(File root) throws UnsupportedEncodingException, IOException, DocumentException, ResultsException {
