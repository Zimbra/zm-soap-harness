...
public class TestCase extends AbsTest {
    ...
    /**
     * NO_TYPE: Default value for all tests w/o a type definition always: always
     * execute the test (ping test, authentication) bhr: bhr test cases smoke: basic
     * tests to smoke test an installed build as 'sane' functional: tests of
     * requirements and core functionality feature: tests of features that are not
     * requirements, but are not negative tests either negative: negative tests
     * deprecated: tests that are no longer applicable to the implementation
     **/
    public static final String[] TYPES = { "NO_TYPE", "always", "smoke", "bhr", "sanity", "functional", "feature",
            "negative", "measurement", "full", "deprecated", "smoke-temp", "bhr-temp", "sanity-temp",
            "functional-temp" };

    public static final String[] DURATIONS = { "short", "long" };
    ...
    public String getType() {
        ...
        try {
            throw (new HarnessException(
                    "Given value for type (" + type + ") attribute does not match expected value."));
    ...
    // supported in
    public boolean installPathShouldSkip(SoapTestCore core) throws HarnessException {
        ...
        if (SoapTestCore.installHistory == null) {
            throw new HarnessException(
                    "Need to define .install_history property, i.e. <t:system command=\"cat\" parms=\"/opt/zimbra/.install_history\" stdout=\".install_history\"/>");
        }
        ...
        while (i.hasNext()) {
            ...
            int release = releaseToInt(Integer.parseInt(parts[0]), Integer.parseInt(parts[1]),
    ...
    protected boolean shouldSkip(SoapTestCore core) throws HarnessException {
        ...
        if (hasArea("selfcheck".split(","))) {
            ...
            if (SoapTestCore.testAreas == null) {
                // Skip this test - no AREAS were specified, so the selfcheck is not being
                // executed
                return (true);
            }
        ...
        if (getHostCount() > SoapTestCore.hostCount) {
            mLog.debug(
                    "shouldSkip: The required hosts for the test case are more than the SUT has.  Don't run the test.");
            return (true);
        }

        // If any excludes are specified, and the excludes match the test case, then
        // skip
        ...
        if ((SoapTestCore.testAreas != null) && (!SoapTestCore.testAreas.isEmpty())) {
            mLog.debug(
                    "shouldSkip: If the test is not a TestCase, then don't run.  If it is a TestCase, run it if the area matches");
        ...
        if (SoapTestCore.testServerBits.equalsIgnoreCase("open")) {
            ...
            // Use "network" when a feature is available for both network and OSS, but
            // behaves differently for network
            String[] networkAreas = { "network", "verity", "backup", "restore", "cluster", "mapi", "crossMailboxSearch",
                    "HSM", "domainAdmin" };
        ...
        if (SoapTestCore.testServerBits.equalsIgnoreCase("network")) {
            ...
            // Use "open" when a feature is available for both network and OSS, but behaves
            // differently for OSS
