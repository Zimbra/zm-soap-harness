    ...
    protected boolean setUri() throws HarnessException {
        ...
        if (server != null) {
            ...
            if (mNamespace.equals(AdminConstants.NAMESPACE_STR)) {
                String httpMode = TestProperties.testProperties.getProperty("zdesktopuser.soapservice.admin.mode",
                        "http");
                String httpPort = TestProperties.testProperties.getProperty("zdesktopuser.soapservice.admin.port",
                        "7633");
                String httpPath = TestProperties.testProperties.getProperty("zdesktopuser.soapservice.admin.path",
                        "service/admin/soap/");
                uriString = httpMode + "://" + server + ":" + httpPort + "/" + httpPath;
            } else {
                String httpMode = TestProperties.testProperties.getProperty("zdesktopuser.soapservice.soap.mode",
                        "http");
                String httpPort = TestProperties.testProperties.getProperty("zdesktopuser.soapservice.soap.port",
                        "7633");
                String httpPath = TestProperties.testProperties.getProperty("zdesktopuser.soapservice.soap.path",
                        "service/soap/");
                uriString = httpMode + "://" + server + ":" + httpPort + "/" + httpPath;
            }
        } else {
            uriString = TestProperties.testProperties.getProperty("zdesktopuser.uri",
                    "http://localhost:7633/service/soap/");
        }

        if (uriString.equals(""))
            throw new HarnessException("uri must be set");

        if (mUri.equals(uriString))
            return (false);
    ...
    public static String getDesktopPassword(String desktopHostname, String defaultPassword) {
        ...
        if ((zDesktopPassword == null) || (zDesktopHost == null) || (!zDesktopHost.equalsIgnoreCase(desktopHostname))) {
            ...
            if (PingStaf()) {
                ...
                String[] possibleFiles = { "/opt/zmdesktop/zimbra/zdesktop/conf/localconfig.xml",
