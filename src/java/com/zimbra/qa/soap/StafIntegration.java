...
//	protected String fSoapLogDir = null;
	...
	public STAFResult init(STAFServiceInterfaceLevel30.InitInfo info)
    {
        ...
        // EXECUTE <zimbra server> < SMOKE | SANITY | FUNCTIONAL | REGRESSION | FULL | DIRECTORY <DIR> | FILE <FILE> | TESTCASE <ID> > [ AREAS | BITS | HOSTS | TYPES ]
...
//        	int numOptions = parsedRequest.optionTimes(aEXECUTE);
        ...
        if ( parsedRequest.optionTimes(aSUITE) > 0 ) {
			...
			} else if ( value.equalsIgnoreCase("REGRESSION") ) {
                SoapTestCore.testType = new ArrayList<String>(Arrays.asList("regression".split(",")));
