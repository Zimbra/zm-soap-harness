...
//	protected String fSoapLogDir = null;
    ...
    public STAFResult init(STAFServiceInterfaceLevel30.InitInfo info) {
        ...
        // EXECUTE <zimbra server> < SMOKE | BHR | SANITY | FUNCTIONAL | FULL |
        // DIRECTORY <DIR> | FILE <FILE> | TESTCASE <ID> > [ AREAS | BITS | HOSTS |
        // TYPES ]
        fExecuteParser = new STAFCommandParser();

        // EXECUTE
        fExecuteParser.addOption(aEXECUTE, 1, STAFCommandParser.VALUEREQUIRED); // argument is the server under test

        fExecuteParser.addOption(aDOMAIN, 1, STAFCommandParser.VALUEREQUIRED); // argument is the default domain name

        fExecuteParser.addOption(aADMIN, 1, STAFCommandParser.VALUEREQUIRED); // argument is the server where admin is
                                                                              // configured

        // Need the ZimbraQA root to get to global.properties
        fExecuteParser.addOption(aZIMBRAQAROOT, 1, STAFCommandParser.VALUEREQUIRED);

        // Types of EXECUTE - one of SUITE, DIRECTORY, FILE, or TESTCASE
        fExecuteParser.addOption(aSUITE, 1, STAFCommandParser.VALUEREQUIRED); // No arg for SMOKE

        fExecuteParser.addOption(aDIRECTORY, 1, STAFCommandParser.VALUEREQUIRED);

        fExecuteParser.addOption(aTESTCASE, 1, STAFCommandParser.VALUEREQUIRED);

        // Package name for soap wsdl test cases.
        fExecuteParser.addOption(aWSTEST, 1, STAFCommandParser.VALUEREQUIRED);

        // Package name for soap wsdl test cases.
        fExecuteParser.addOption(aRUNWSDLTESTS, 1, STAFCommandParser.VALUEREQUIRED);

        // Optional LOG argument
        fExecuteParser.addOption(aLOGDIR, 1, STAFCommandParser.VALUEREQUIRED);

        // Optional LOG4J argument
        fExecuteParser.addOption(aLOG4JPROPERTIESFILE, 1, STAFCommandParser.VALUEREQUIRED);

        // Optional test parameters
        // TODO: Ideally, areas and types should be allowed to appear more than once
        // TODO: however, I don't want to have to code the parsing of multiple areas or
        // types
        fExecuteParser.addOption(aSETUP, 1, STAFCommandParser.VALUEREQUIRED);

        fExecuteParser.addOption(aAREAS, 1, STAFCommandParser.VALUEREQUIRED);

        fExecuteParser.addOption(aEXCLUDES, 1, STAFCommandParser.VALUEREQUIRED);

        fExecuteParser.addOption(aBITS, 1, STAFCommandParser.VALUEREQUIRED);

        fExecuteParser.addOption(aHOSTS, 1, STAFCommandParser.VALUEREQUIRED);

        fExecuteParser.addOption(aTYPES, 1, STAFCommandParser.VALUEREQUIRED);

        fExecuteParser.addOption(aDURATION, 1, STAFCommandParser.VALUEREQUIRED);

        fExecuteParser.addOption(aTESTSUITES, 1, STAFCommandParser.VALUEREQUIRED);
    ...
    public STAFResult acceptRequest(STAFServiceInterfaceLevel30.RequestInfo info) {
        ...
        if (request.equals(aEXECUTE.toLowerCase())) {
            ...
            return new STAFResult(STAFResult.InvalidRequestString,
                    "Unknown SoapTestCore (STAF) Request: " + lowerRequest);
        }
    }
    ...
    private STAFResult handleHelp() {
        ...
        // etc.
        return new STAFResult(STAFResult.Ok, "StafTest Service Help" + fLineSep + fLineSep
                + "EXECUTE <Zimbra Server> [ DOMAIN <Default Domain> ] [ ADMIN <Admin Server> ] " + fLineSep
                + "ZIMBRAQAROOT <full path> ( SUITE <smoke | full> | DIRECTORY <full path> | WSTEST <Package Name like com.zimbra.qa.soapws.tests  OR test class name like  com.zimbra.qa.soapws.tests.admin.BasicAccountRequestTest >) "
                + fLineSep
                + "optional [ TESTCASE <id> | SETUP <full path> | RUNWSDLTESTS <true|false> | |AREAS <a> | BITS <open | network> | HOSTS <#> | TYPES <t> | LOG <dir path> | LOG4J <prop file> "
                + fLineSep + fLineSep + "QUERY <job> -- TBD: should return statistics on active jobs " + fLineSep
                + fLineSep + "VERSION" + fLineSep + fLineSep + "HALT (not supported)" + fLineSep + fLineSep + "HELP");
    }
    ...
    private STAFResult handleExecute(STAFServiceInterfaceLevel30.RequestInfo info) {
        ...
        if (info.trustLevel < 4) {
            return new STAFResult(STAFResult.AccessDenied, "Trust level 4 required for EXECUTE request." + fLineSep
        ...
        try {
            ...
            if (SoapTestMain.sHarnessTestCases == null && SoapTestMain.sHarnessTestSuite == null
                    && SoapTestMain.testPackageOrClassName.equals("")) {
            ...
            try {
                ...
                mLog.info("\n\nTest finished: "
                        + (SoapTestMain.mTotalTestCasePass + SoapTestMain.mTotalTestCaseFail > 0 ? "FAIL" : "PASS"));
            } catch (Exception e) {
                ...
                resultString.append("\n");
                resultString.append(
                        "Executed:" + (SoapTestMain.mTotalTestCasePass + SoapTestMain.mTotalTestCaseFail) + "\n");
                ...
                resultString.append("\n\nTest finished: "
                        + ((SoapTestMain.mTotalTestCaseFail + SoapTestMain.mTotalTestCaseError > 0) ? "FAIL" : "PASS")
                        + "\n");

                // Write the pass/fail results to file
                writeTestSummaryTxt(SoapTestCore.mLogDirectory, SoapTestMain.mTotalTestCasePass,
                        SoapTestMain.mTotalTestCaseFail, SoapTestMain.mTotalTestCaseError);
                ...
                resultString.append(
                        "======================================================================================\n");
                resultString.append("Rerun Executed:"
                        + (SoapTestMain.mTotalRerunTestCasePass + SoapTestMain.mTotalRerunTestCaseFail) + "\n");
                ...
                resultString.append("\n\nRerun Test finished: "
                        + ((SoapTestMain.mTotalRerunTestCaseFail + SoapTestMain.mTotalRerunTestCaseError > 0) ? "FAIL"
                                : "PASS")
                        + "\n");

                // Write the pass/fail results to file
                writeTestSummaryTxt(SoapTestCore.mLogDirectory, SoapTestMain.mTotalRerunTestCasePass,
                        SoapTestMain.mTotalRerunTestCaseFail, SoapTestMain.mTotalRerunTestCaseError);
    ...
    private void parseExecute(STAFCommandParseResult parsedRequest) {
        ...
        if (parsedRequest.optionTimes(aLOGDIR) > 0) {
            ...
            try {
                ...
                FileAppender appender = FileAppender.newBuilder().setName("file")
                        .setLayout(PatternLayout.newBuilder().withPattern("%m%n").build()).withFileName(logFileName)
                        .build();
        ...
        if (parsedRequest.optionTimes(aEXECUTE) > 0) {
            ...
            // Additional server names can be specified for the other multihost servers (not
            // implemented)
            //
...
//        	int numOptions = parsedRequest.optionTimes(aEXECUTE);
            ...
            // String zimbraDefaultDomain = ( (parsedRequest.optionTimes(aEXECUTE) > 1) ?
            // parsedRequest.optionValue(aEXECUTE, 2) : fSoapZimbraServerName );
        ...
        // This must come after aEXECUTE, since that will specify the server name and
        // default domain
    ...
    private STAFResult handleQuery(STAFServiceInterfaceLevel30.RequestInfo info) {
        ...
        if (info.trustLevel < 2) {
            return new STAFResult(STAFResult.AccessDenied, "Trust level 2 required for QUERY request. Requesting "
        ...
        if (!fAlreadyRunning) {
            ...
            resultString.append("In Progress:" + fLineSep + "Executed:"
                    + (SoapTestMain.mTotalTestCasePass + SoapTestMain.mTotalTestCaseFail) + fLineSep + "Pass:"
                    + SoapTestMain.mTotalTestCasePass + fLineSep + "Fail:" + SoapTestMain.mTotalTestCaseFail + fLineSep
            ...
            resultString.append("In Progress:" + fLineSep + "Rerun Executed:"
                    + (SoapTestMain.mTotalRerunTestCasePass + SoapTestMain.mTotalRerunTestCaseFail) + fLineSep + "Pass:"
                    + SoapTestMain.mTotalRerunTestCasePass + fLineSep + "Fail:" + SoapTestMain.mTotalRerunTestCaseFail
                    + fLineSep + "Script Errors:" + SoapTestMain.mTotalRerunTestCaseError + fLineSep);
            resultString.append(fLineSep);
    ...
    private String resolveSystemVar(String variable) {
        ...
        if ((variable != null) && (!variable.equals(""))) {
            STAFResult varResult = fHandle.submit2("local", "var", "get system var " + variable);
            if (varResult.rc == 0) {
