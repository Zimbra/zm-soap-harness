...
public class INJECTStaf implements STAFServiceInterfaceLevel30 {
    ...
    public STAFResult acceptRequest(RequestInfo info) {
        ...
        if (request.equalsIgnoreCase(SMTP)) {
            ...
            return new STAFResult(STAFResult.InvalidRequestString, "Unknown INJECT (STAF) Request: " + request);
        }
    }
    ...
    protected STAFResult smtpExecute(SmtpConfig config) {
        ...
        EmailImpl smtp = new EmailImpl(config.host, config.port, config.user, config.password, config.auth, config.ssl,
                config.startls);
        ...
        return new STAFResult(STAFResult.Ok,
                baos == null ? "SMTP trace not logged - message size larger than " + FileSizeLimit
                        : smtp.getServerResponse());
    }
    ...
    protected void createSmtpParser() {
        ...
        /*
         * SMTP TO <Address>... FROM <user@company.com> SUBJECT <Subject> MESSAGE
         * <Message File> FILE <File> TEXTATTACHMENT <file> BINARYATTACHMENT <file>
         * FSERVER <Machine where the msg/file is stored> AUTH USER < user name used
         * only when you specify auth> PASSWD < password used only when you specify
         * auth> PORT <port> HOST <host> SSL STARTLS
         */
