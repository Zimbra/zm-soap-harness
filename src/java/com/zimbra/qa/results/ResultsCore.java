...
public class ResultsCore {
    ...
    protected String recordResults() throws ResultsException {
        ...
        columns.append(" passed");			values.append(" " + vPassed);
        columns.append(", failed");			values.append(", " + vFailed);
        columns.append(", error");			values.append(", " + vError);
                
        if ( getSuiteID(true) != null ) {
            columns.append(", suite_id");		values.append(", " + getSuiteID(true));
        }
        
        if ( getArchitectureID(true) != null ) {
            columns.append(", architecture_id");	values.append(", " + getArchitectureID(true));
        }
        
        if (vURL != null) {
        	columns.append(", url"); values.append(", '" + vURL + "'");
        }
        	
/*
 * Don't use bits for now
 * 
        if ( getBitsName(true) != null ) {
            columns.append(", bits");		values.append(", " + q(getBitsName(true)));
        }
 * 
 */        
        
        if ( getBuildID(true) != null ) {
            columns.append(", build_id");	values.append(", " + getBuildID(true));
        }
        
        if ( getTypeID(true) != null ) {
            columns.append(", testtype_id");	values.append(", " + getTypeID(true));
        }
        
        if ( getBranchID(true) != null ) {
            columns.append(", branch_id");	values.append(", " + getBranchID(true));
        }
        
    	String insert = new String(
    			"INSERT INTO " + dResultsTable + " " +
    			" ( " + columns.toString() + " ) " +
    			" VALUES " +
    			" ( " + values.toString() + " ) ");
 ...
 * Don't use bits for now
 * 
            if ( getBitsName(false) != null ) {
            	where.append( " AND " + p("bits = "+ q(getBitsName(false))) );
            }
 * 
 */            
            ...
            String query = new String(
	    			"SELECT build_id, passed, failed, error FROM  " + dResultsTable + 
	    			" WHERE " + p(where.toString()) + ";"
	    			);
	    	...
	    	while ( rs.next() ) {
	    		...
	    		mLog.info(getBuildName(rs.getString("build_id")) + ": " +
	    				"Passed(" + rs.getString("passed") + ") " +
	    				"Failed(" + rs.getString("failed") + ") " +
	    				"Script Errors(" + rs.getString("error") + ")");
	    		
	    		result.append(
	    				getBuildName(rs.getString("build_id")) + ": " +
	    				"Passed(" + rs.getString("passed") + ") " +
	    				"Failed(" + rs.getString("failed") + ") " +
	    				"Script Errors(" + rs.getString("error") + ")\n"
	    				);
