...
public class StafMain implements STAFServiceInterfaceLevel30 {
    ...
    public STAFResult init(STAFServiceInterfaceLevel30.InitInfo info) {
        ...
        fMainParser.addOption(sMain, 1, STAFCommandParser.VALUEREQUIRED); // argument is the server under test
        ...
        fHaltParser.addOption(sHALT, 1, STAFCommandParser.VALUENOTALLOWED);
        ...
        // Set up the Logger
        try {
            ConsoleAppender consoleAppender = ConsoleAppender.newBuilder().setName("console")
                    .setTarget(ConsoleAppender.Target.SYSTEM_OUT).build();
            FileAppender fileAppender = FileAppender.newBuilder().setName("file")
                    .setLayout(PatternLayout.newBuilder().withPattern(PatternLayout.DEFAULT_CONVERSION_PATTERN).build())
                    .withFileName(mLogFileName).build();
    ...
    public STAFResult acceptRequest(STAFServiceInterfaceLevel30.RequestInfo info) {
        ...
        if (request.equals(sMain.toLowerCase())) {
            ...
            return new STAFResult(STAFResult.InvalidRequestString, "Unknown StafCore (STAF) Request: " + lowerRequest);
        }
    }
    ...
    private STAFResult handleHelp() {
        ...
        return new STAFResult(STAFResult.Ok,
                "StafTest Service Help " + fLineSep + fLineSep + "MAIN <string> " + fLineSep + fLineSep
                        + "QUERY <job> -- TBD: should return statistics on active jobs " + fLineSep + fLineSep
                        + "VERSION" + fLineSep + fLineSep + "HALT (not supported) -- TBD: should stop all active jobs "
                        + fLineSep + fLineSep + "HELP");
    }
