...
public class StafIntegration implements STAFServiceInterfaceLevel30  {
	...
	public STAFResult init(STAFServiceInterfaceLevel30.InitInfo info)
    {
        ...
        // "ACCOUNT INPUTFILE <filepath> < DOMAIN <account domain> | EMAIL <account email> > [ PASSWORD <account password> ] [ SERVER <zimbra server> ]"; 
        ...
        fAccountParser.addOptionGroup(aSERVER +" "+ aDOMAIN +" "+ aEMAIL, 1, 2); // Must specify SERVER OR specify one of DOMAIN or EMAIL
    ...
    {               
        ...
        {
            return new STAFResult(STAFResult.InvalidRequestString, "Unknown SoapTestCore (STAF) Request: " +  info.request);
        }
    }
    ...
    private STAFResult handleHelp()
    {
        ...
        return new STAFResult(STAFResult.Ok,
         "ACCOUNT INPUTFILE {filepath} < DOMAIN {account domain} | EMAIL {account email} >  [ SERVER {zimbra server} ] [ PASSWORD {account password} ] " + fLineSep + fLineSep +
         "where" + fLineSep +
         "\t{filepath} is a text file containing the output of 'zmprov ga account@domain.com'" + fLineSep +
         ...
         "HALT (not supported)" + fLineSep + fLineSep + 
         "HELP");
    }
    ...
    private STAFResult handleAccount(STAFServiceInterfaceLevel30.RequestInfo info) 
    {
        ...
        {   
        	...
        	return new STAFResult(STAFResult.AccessDenied, 
                "Trust level 4 required for "+ aACCOUNT +" request." + fLineSep +
