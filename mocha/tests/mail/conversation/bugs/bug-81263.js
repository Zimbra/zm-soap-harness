...
describe('Mail > Conversation > Bugs > Bug 81263', function () {
	...
	it('Sanity | Login as the test account', async () => {
		...
		// GetConvRequest and verify email address
		...
		const emails = Array.isArray(getConv.e)
			? getConv.e : [getConv.e];
		const matchingEmail = emails.find(e => e && e.a === accountEmail);
		assert.exists(matchingEmail, `Should find ${accountEmail} in conversation emails`);
	});
});
