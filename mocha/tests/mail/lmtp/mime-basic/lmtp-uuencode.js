...
describe('Mail > LMTP > MIME Basic > LMTP Uuencode', function () {
	...
	// Tests
	it('Regression | inject a mime message with a uuencoded attachment using LMTP 1', async () => {
		// Inject MIME message
		const filePath = path.join(config.data, 'uuencode/uuencode01.txt');
	...
	it('Regression | inject a mime message with a uuencoded attachment using LMTP 2', async () => {
		// Inject MIME message
		const filePath = path.join(config.data, 'uuencode/uuencode02.txt');
		await soap.injectMime(accountAuthToken, filePath);
