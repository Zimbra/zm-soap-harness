...
describe('Mail > LMTP > MIME Basic > LMTP Basic', function () {
	...
	// Tests
	it('Sanity | inject a basic mime message using LMTP', async () => {
		// Inject MIME message
	...
	it('Sanity | Verify the display name that is built from the mime message', async () => {
		// Inject MIME message
		const filePath = path.join(config.data, 'tests/lmtp-basic01.txt');
		await soap.injectMime(accountAuthToken, filePath);
