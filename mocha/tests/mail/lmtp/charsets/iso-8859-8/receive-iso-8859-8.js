...
describe('Mail > LMTP > Charsets > ISO 8859 8 > Receive ISO 8859 8', function () {
	...
	// Tests
	it('Sanity | Verify account can receive iso 8859-8 encoded content', async () => {
		// Inject MIME message
	...
	it('Sanity | Verify account can receive iso 8859-8 encoded subject', async () => {
		// Inject MIME message
		const filePath = path.join(config.data, 'tests/lmtp-basic01.txt');
		await soap.injectMime(accountAuthToken, filePath);
	...
	it('Sanity | Verify account can receive iso 8859-8 encoded recipient', async () => {
		// Inject MIME message
		const filePath = path.join(config.data, 'tests/lmtp-basic01.txt');
		await soap.injectMime(accountAuthToken, filePath);
