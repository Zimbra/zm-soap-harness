...
describe('Mail > LMTP > Charsets > ISO 8859 8 I > Msg ISO 8859 8 I', function () {
	...
	// Tests
	it('Sanity | Verify account can receive iso 8859-8i encoded content', async () => {
		// Inject MIME message
	...
	it('Sanity | Verify account can receive iso 8859-8i encoded subject', async () => {
		// Inject MIME message
		const filePath = path.join(config.data, 'tests/lmtp-basic01.txt');
		await soap.injectMime(accountAuthToken, filePath);
