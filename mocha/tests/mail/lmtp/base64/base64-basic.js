...
describe('Mail > LMTP > Base64 > Base64 Basic', function () {
	...
	// Tests
	it('Sanity | Verify a Base64 message is decoded correctly (Fragment and content)', async () => {
		// Inject MIME message
	...
	it('Functional | Verify a Base64 message is searchable', async () => {
		// Inject MIME message
		const filePath = path.join(config.data, 'tests/lmtp-basic01.txt');
		await soap.injectMime(accountAuthToken, filePath);
