...
describe('Mail > LMTP > Mac > Mac MIME Basic', function () {
	...
	// Tests
	it('Sanity | Verify a MIME message in text format from a Mac Account', async () => {
		// Inject MIME message
		const filePath = path.join(config.data, 'tests/mac-mail-basic.txt');
	...
	it('Sanity | Verify a MIME message with Word Document Attached from a Mac Account', async () => {
		// Inject MIME message
		const filePath = path.join(config.data, 'tests/mac-mail-attachment.txt');
		await soap.injectMime(accountAuthToken, filePath);
