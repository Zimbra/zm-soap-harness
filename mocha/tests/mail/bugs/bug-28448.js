...
describe('Mail > Bugs > Bug 28448', function () {
	...
	// Tests
	it('Sanity | Checking if i am getting a response to the injected message', async () => {
		// Create test account
		...
		const filePath = path.join(config.projectRoot, 'mocha/data/bugs/28448/msg01.txt');
		await soap.injectMime(authToken, filePath);

		// Search for the injected message
		const searchRes = await soap.makeSOAPEnvelopeAccount(
			`<SearchRequest xmlns="urn:zimbraMail" types="message">
				<query>message1</query>
		...
		// Get the message and verify subject and attachment filename
		...
		assert.include(getMsg.su, 'message1', 'Subject should match');
		const parts = JSON.stringify(getMsg.mp);
		assert.include(parts, 'filename', 'Attachment should have filename');
	});
});
