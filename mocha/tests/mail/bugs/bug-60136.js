...
describe('Mail > Bugs > Bug 60136', function () {
	...
	it('Sanity | Inject the mime and verify that mht file is rendered', async () => {
		...
		// Add a message with an embedded message/rfc822 part
...
MIME-Version: 1.0
Content-Type: multipart/mixed; boundary="boundary60136"
...
&lt;html&gt;&lt;body&gt;Inner HTML content&lt;/body&gt;&lt;/html&gt;
--boundary60136--
</content>
				</m>
		...
		// Get the message and verify it has message/rfc822 part
	...
	it('Sanity | Inject mime and verify print message renders properly', async () => {
		// Create the account
		const accountEmail = `test${common.getUniqueString()}@${testDomain}`;
		await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
...
&lt;html&gt;&lt;body&gt;NPC Order content&lt;/body&gt;&lt;/html&gt;
</content>
				</m>
			</AddMsgRequest>`, authToken
		);
		...
		// Get the message to verify it displays properly
		const getMsgRes = await soap.makeSOAPEnvelopeAccount(
			`<GetMsgRequest xmlns="urn:zimbraMail">
				<m id="${msgId}"/>
			</GetMsgRequest>`, authToken
