...
describe('Mail > Bugs > Bug 27365', function () {
	...
	it('Sanity | Checking if i am getting a response to the injected message', async () => {
		...
		const filePath = path.join(config.projectRoot, 'mocha/data/bugs/27365/msg01.txt');
		await soap.injectMime(authToken, filePath);

		// Search for the message by content
		...
		assert.include(
			getMsg.su,
			'IIE.Interactive:  March 31-April 4, 2008',
