...
describe('Mail > Bugs > Bug 28675', function () {
	...
	// Tests
	it('Sanity | Verify bug 28675', async () => {
		// Create test account
		...
		// Inject the MIME message with winmail.dat attachment
		const filePath = path.join(config.projectRoot, 'mocha/data/bugs/28675/bug28675.txt');
		...
		// Get the message and verify winmail.dat attachment
		...
		assert.include(parts, 'application/vnd.ms-tnef', 'Should have ms-tnef content type');
		assert.include(parts, 'winmail.dat', 'Should have winmail.dat filename');
	});
});
