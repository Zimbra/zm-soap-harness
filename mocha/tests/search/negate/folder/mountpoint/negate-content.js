...
describe('Search > Negate > Folder > Mountpoint > Negate Content', function () {
	...
	before(async function () {
		...
		// Inject test message into account1's inbox
...
Test content for searching
	...
	// Tests
	it('Sanity | Verify a search for not content - (text) in a mountpoint', async () => {
		// SearchRequest
	...
	it('Sanity | Verify a search for not content - (text) in a remote folder', async () => {
		// SearchRequest
		const res1 = await soap.makeSOAPEnvelopeAccount(
			`<SearchRequest xmlns="urn:zimbraMail" types="message">
