...
describe('Admin > Accounts > Get Account Multinode', function () {
    ...
    it('Sanity | GetAccountRequest is successfully proxied across hosts', async () => {
        const account1Name = 'hostA' + common.getUniqueString() +
            '@' + config.testDomain;
        const account2Name = 'hostB' + common.getUniqueString() +
            '@' + config.testDomain;
        ...
        const mailHost2 = acct2.a.find(
            a => a.n === 'zimbraMailHost');
        assert.equal(mailHost2._content, serverBName,
            'Account 2 should be on server B');
        ...
        assert.exists(get1Res.GetAccountResponse,
            'GetAccountResponse for account 1 should exist');
        ...
        assert.exists(get2Res.GetAccountResponse,
            'GetAccountResponse for account 2 should exist');
    });
});
