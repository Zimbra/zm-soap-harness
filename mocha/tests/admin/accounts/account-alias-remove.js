...
describe('Admin > Accounts > Account Alias Remove', function () {
	...
	it('Regression | Remove an invalid alias (without domain name)', async () => {
		const response = await soap.makeSOAPEnvelopeAdmin(
			`<RemoveAccountAliasRequest xmlns="urn:zimbraAdmin">
				<id>${account1Id}</id>
	...
	it('Regression | Remove an invalid alias (with non existing domain name)', async () => {
		const response = await soap.makeSOAPEnvelopeAdmin(
			`<RemoveAccountAliasRequest xmlns="urn:zimbraAdmin">
				<id>${account1Id}</id>
	...
	it('Regression | Delete an account. The alias of that account added in DL should also go from DL.', async () => {
		await soap.makeSOAPEnvelopeAdmin(
			`<AddAccountAliasRequest xmlns="urn:zimbraAdmin">
				<id>${account3Id}</id>
