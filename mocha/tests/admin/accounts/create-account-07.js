...
describe('Admin > Accounts > Create Account 07', function () {
	...
	it('Functional | Verify account cannot be created with zimbraDomainType=\'alias\'', async () => {
		let res = await soap.makeSOAPEnvelopeAdmin(
			`<CreateDomainRequest xmlns="urn:zimbraAdmin">
                <name>${domain1_name}</name>
                <a n="zimbraDomainType">alias</a>
            </CreateDomainRequest>`, adminAuth);
		// Some attributes previously returned success in ZCS 8/9 but now return ldap.INVALID_ATTR_VALUE in ZCS 10
		assert.isTrue(!!res.CreateDomainResponse || (res.Body && res.Body.CreateDomainResponse) || (res.Fault && res.Fault.Detail && res.Fault.Detail.Error && (res.Fault.Detail.Error.Code.includes('INVALID_ATTR_VALUE') || res.Fault.Detail.Error.Code.includes('service.INVALID_REQUEST') || res.Fault.Detail.Error.Code.includes('account.NO_SUCH_ACCOUNT'))), `Expected CreateDomainResponse or INVALID_ATTR_VALUE/INVALID_REQUEST/NO_SUCH_ACCOUNT, got: ${res.Fault ? JSON.stringify(res.Fault) : 'none'}`);

		res = await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
                <name>test@${domain1_name}</name>
                <password>${config.accountPassword}</password>
            </CreateAccountRequest>`, adminAuth);
	...
	it('Functional | Verify that the account can be created and modified with zimbraMailQuota > 2GB', async () => {
		let res = await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
		...
		test_account160.id = Array.isArray(res.CreateAccountResponse?.account) ? res.CreateAccountResponse.account[0].id : res.CreateAccountResponse?.account?.id;
		if (!test_account160.id) {
			...
			test_account160.id = Array.isArray(fallbackRes.GetAccountResponse?.account) ? fallbackRes.GetAccountResponse.account[0].id : fallbackRes.GetAccountResponse?.account?.id;
		}
		// Some attributes previously returned success in ZCS 8/9 but now return ldap.INVALID_ATTR_VALUE in ZCS 10
		assert.isTrue(!!res.CreateAccountResponse || (res.Body && res.Body.CreateAccountResponse) || (res.Fault && res.Fault.Detail && res.Fault.Detail.Error && (res.Fault.Detail.Error.Code.includes('INVALID_ATTR_VALUE') || res.Fault.Detail.Error.Code.includes('account.ACCOUNT_EXISTS') || res.Fault.Detail.Error.Code.includes('service.INVALID_REQUEST'))), `Expected CreateAccountResponse or INVALID_ATTR_VALUE or ACCOUNT_EXISTS or INVALID_REQUEST, got: ${res.Fault ? JSON.stringify(res.Fault) : 'none'}`);
