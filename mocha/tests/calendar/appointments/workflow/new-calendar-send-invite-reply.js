...
describe('Calendar > Appointments > Workflow > New Calendar Send Invite Reply', function () {
	...
	it('Smoke | Accepting invitation request', async () => {
		// Create accounts
	...
	it('Smoke | Declining invitation request', async () => {
		// Create accounts
		const org = await makeAcct('org');
		const inv = await makeAcct('inv');
		const subject = `Subj${common.getUniqueString()}`;
	...
	it('Smoke | Verify the mails for accept, tentative, decline of invitation are received properly', async () => {
		// Create accounts
		const org = await makeAcct('org');
		const inv = await makeAcct('inv');
		const subject = `Subj${common.getUniqueString()}`;
