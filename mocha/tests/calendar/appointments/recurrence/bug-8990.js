...
describe('Calendar > Appointments > Recurrence > Bug 8990', function () {
	...
	it('Sanity | Verify recurring with BYMONTHDAY 31 happens on all months', async () => {
		// Create account
	...
	it('Sanity | Verify recurring with BYMONTHDAY -1 should happen on last day of every month', async () => {
		// Create account
		const acct = await makeAcct('rec');
		const s = `Subj${common.getUniqueString()}`;
		const appt = await createRecurAppt(
