...
describe('Folders > Bugs > Bug 40759', function () {
    ...
    before(async function () {
        ...
        for (let i = 0; i < 4; i++) {
            const name = `bug40759_acct${i}_${common.getUniqueString()}@${config.testDomain}`;
    ...
    after(async function () {
        ...
        if (dls.length > 0) {
            for (const dlId of dls) {
                ...
                const deleteDl = `<DeleteDistributionListRequest xmlns="urn:zimbraAdmin"><id>${dlId}</id></DeleteDistributionListRequest>`;
    ...
    it('Regression | Bug 40759_01 | Verify error does not occur when mounting folder shared with DL', async () => {
        ...
        const aliasName = `alias_bug40759_1_${common.getUniqueString()}@${config.testDomain}`;
        const addAlias =
        ...
        assert.exists(mountResp.CreateMountpointResponse.link[0].id, 'Mountpoint created successfully');
    });


    it('Regression | Bug 40759_02 | Verify mountpoint with DL alias grant', async () => {
        ...
        const aliasName = `alias_bug40759_2_${common.getUniqueString()}@${config.testDomain}`;
        const addAlias =
            `<AddDistributionListAliasRequest xmlns="urn:zimbraAdmin">
                <id>${dlId}</id>
                <alias>${aliasName}</alias>
        ...
        const aliasName2 = `alias_bug40759_2b_${common.getUniqueString()}@${config.testDomain}`;
        const addAlias2 =
            `<AddDistributionListAliasRequest xmlns="urn:zimbraAdmin">
                <id>${dlId}</id>
