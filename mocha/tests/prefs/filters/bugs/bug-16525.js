...
describe('Prefs > Filters > Bugs > Bug 16525', function () {
	...
	// Tests
	it('Sanity | Verify bug 16525 - Filter a mail with invalid mime', async () => {
	...
	it('Functional | Verify bug 16525 - Filter a mail with invalid mime (Content-Type - text charset utf-8)', async () => {
		const accountEmail = `test.${common.getUniqueString()}@${testDomain}`;
		await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
				<name>${accountEmail}</name>
