...
describe('Prefs > Filters > Bugs > Bug 65369', function () {
	...
	// Tests
	it('Sanity | Verify Filter Action Notify has charset us-ascii if the message body contain ASCII only', async () => {
	...
	it('Sanity | Verify Filter Action Notify has charset=utf-8 if the message body is not ASCII only and the message char code includes bigger char code than zimbraPrefMailDefaultCharset', async () => {
		const accountEmail = `test.${common.getUniqueString()}@${testDomain}`;
		const createAcctRes = await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
				<name>${accountEmail}</name>
	...
	it('Sanity | Verify Filter Action Notify has charset=iso-2022-jp if the message body is not ASCII only and the message char code includes bigger char code than zimbraPrefMailDefaultCharset is set Japanese(iso-2022-jp)', async () => {
		const accountEmail = `test.${common.getUniqueString()}@${testDomain}`;
		const createAcctRes = await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
				<name>${accountEmail}</name>
