...
describe('Prefs > Filters > Bugs > Bug 35983', function () {
	...
	// Tests
	it('Sanity | Test to see if more than 4 consecutive "*" is possible in Filter wildcard matches and also to check if delivery of messages hang', async () => {
	...
	it('Sanity | Create wildcard rule to check if the email gets filtered for 4 consecutive star', async () => {
		const accountEmail = `test.${common.getUniqueString()}@${testDomain}`;
		await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
				<name>${accountEmail}</name>
