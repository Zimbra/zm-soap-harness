...
describe('Prefs > Filters > Bugs > Bug 45851', function () {
	...
	// Tests
	it('Sanity | Apply filter rule to same folder as "file into". Verify no duplicates. 1', async () => {
	...
	it('Sanity | Apply filter rule to same folder as "file into". Verify no duplicates. 2', async () => {
		const accountEmail = `test.${common.getUniqueString()}@${testDomain}`;
		await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
				<name>${accountEmail}</name>
