...
describe('Prefs > Filters > Bugs > Bug 77367', function () {
	...
	// Tests
	it('Sanity | Notifyaction filter can result in mail loop', async () => {
	...
	it('Sanity | actionReply filter can result in mail loop', async () => {
		const accountEmail = `test.${common.getUniqueString()}@${testDomain}`;
		await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
				<name>${accountEmail}</name>
