...
describe('Prefs > Filters > Bugs > Bug 19559', function () {
	...
	// Tests
	it('Functional | Verify bug 19102 - Create a mail filter greater than 21 KB 1', async () => {
	...
	it('Functional | Verify bug 19102 - Create a mail filter greater than 21 KB 2', async () => {
		const accountEmail = `test.${common.getUniqueString()}@${testDomain}`;
		const createAcctRes = await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
				<name>${accountEmail}</name>
