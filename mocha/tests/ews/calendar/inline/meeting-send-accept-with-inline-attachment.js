...
describe('EWS > Calendar > Inline > Meeting Send Accept With Inline Attachment', function () {
	...
	// Tests
	it('Sanity | Create and send meeting with inline image from EWS and sync on ZWC client and accept', async () => {
		const unique = common.getUniqueString();
		...
		// Step 1: Create appointment from EWS (SaveOnly, no send yet)
		...
		await ews.makeEWSRequest(
			`<UpdateItem xmlns="http://schemas.microsoft.com/exchange/services/2006/messages"
				...
				<ItemChanges>
					<t:ItemChange>
						<t:ItemId Id="${rootItemId}" ChangeKey="${rootItemCk}" />
						<t:Updates>
							<t:SetItemField>
		...
		// Step 4: Sync and verify on organizer side (ZWC)
		...
		assert.exists(inboxMsg, 'Inbox message should exist for attendee');

		// Search for appointment on attendee side
		...
		assert.exists(attendeeAppt, 'Appointment should exist for attendee');
		const attendeeInvId = attendeeAppt.invId;

		// Step 6: Attendee accepts the meeting
		...
		const responseSearch = await soap.makeSOAPEnvelopeAccount(
			`<SearchRequest xmlns="urn:zimbraMail" types="message">
				<query>in:"Inbox" from:${account2Email} subject:"${apptSubject}"</query>
