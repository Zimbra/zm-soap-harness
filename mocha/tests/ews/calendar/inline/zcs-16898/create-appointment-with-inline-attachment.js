...
// Small base64-encoded PNG for testing inline/attachment scenarios
...
describe('EWS > Calendar > Inline > ZCS-16898 > Create Appointment With Inline Attachment', function () {
	...
	// Tests
	it('Sanity | Create and save appointment with inline image from EWS and sync on ZWC client', async () => {
		const unique = common.getUniqueString();
		...
		const mpArr = msg.mp;
		assert.exists(mpArr, 'Message should have mime parts');

		// Verify inline image attachment exists
		const findInlinePart = (parts) => {
			if (!parts) return null;
			const partsArr = Array.isArray(parts) ? parts : [parts];
			for (const part of partsArr) {
				if (part.ct === 'image/png' && part.cd === 'inline') return part;
				if (part.mp) {
					const found = findInlinePart(part.mp);
					if (found) return found;
				}
			}
			return null;
		};
		const inlinePart = findInlinePart(mpArr);
		assert.exists(inlinePart, 'Should have an inline image/png part');
		assert.equal(inlinePart.cd, 'inline', 'Content disposition should be inline');
		assert.include(inlinePart.ci, 'image001.png', 'Content ID should contain image001.png');
	});
});
