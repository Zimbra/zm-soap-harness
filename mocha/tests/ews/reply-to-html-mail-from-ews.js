...
describe('EWS > Reply To Html Mail From EWS', function () {
	...
	before(async function () {
		...
		account1Email = `ewsrhtml1${common.getUniqueString()}@${config.testDomain}`;
		...
		account2Email = `ewsrhtml2${common.getUniqueString()}@${config.testDomain}`;
		await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
	...
	// Tests
	it('Sanity | Send HTML text mail from user1 To user2 from ZWC', async () => {
