...
describe('EWS > Forward HTML Mail', function () {
	...
	before(async function () {
		...
		account1Email = `ewsfwdhtml1${common.getUniqueString()}@${config.testDomain}`;
		...
		account2Email = `ewsfwdhtml2${common.getUniqueString()}@${config.testDomain}`;
		await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
		...
		account3Email = `ewsfwdhtml3${common.getUniqueString()}@${config.testDomain}`;
		await soap.makeSOAPEnvelopeAdmin(
			`<CreateAccountRequest xmlns="urn:zimbraAdmin">
	...
	it('Sanity | Send HTML mail from user1 To user2 from ZWC and sync on EWS Then forward same mail to user3 from EWS', async () => {
		...
		// EWS: Forward (CreateItem with MimeContent) from account2 to account3
		...
		const mimeContent = Buffer.from(
			...
			'\tboundary="B_3587392187_2338550078"\r\n' +
			'\r\n' +
			'--B_3587392187_2338550078\r\n' +
			...
			'Content-transfer-encoding: 7bit\r\n' +
			'\r\n' +
			`<html><body>${forwardContent}</body></html>\r\n` +
			'\r\n' +
			'--B_3587392187_2338550078--\r\n'
		).toString('base64');
				...
				<ItemShape>
					...
					<t:AdditionalProperties>
						...
						<t:ExtendedFieldURI PropertyTag="0x1090"
							PropertyType="Integer" />
						<t:ExtendedFieldURI PropertyTag="0x1081"
							PropertyType="Integer" />
						<t:ExtendedFieldURI PropertyTag="0x1082"
						...
						<t:ExtendedFieldURI PropertyTag="0xf03"
							PropertyType="Binary" />
						...
						<t:ExtendedFieldURI PropertyTag="0x1213"
							PropertyType="Integer" />
					</t:AdditionalProperties>
				</ItemShape>
				<SyncFolderId>
