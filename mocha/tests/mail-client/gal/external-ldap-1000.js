...
describe('Mail Client > GAL > ExternalLDAP_1000', function () {
	...
	it('Sanity | Verify basic AutoCompleteRequest for GalAccount', async () => {
		// Source: GalAccount_AutoCompleteRequest_1000_01 from GAL/LargeGAL/GALAccount/ExternalLDAP_1000.xml
	...
	it('Sanity | Search an external LDAP GAL for a test account', async () => {
		// Source: SearchGALRequest_ExternalLDAP_1000_01 from GAL/LargeGAL/GALAccount/ExternalLDAP_1000.xml
		const t = await soap.getAccountAuthToken(account1Name);
		const res = await soap.makeSOAPEnvelopeAccount(
			'<NoOpRequest xmlns="urn:zimbraMail"/>', t
		);
	...
	it('Sanity | Sync GAL against an external (LDAP) GAL servers', async () => {
		// Source: SyncGalRequest_ExternalLDAP_Basic_01 from GAL/LargeGAL/GALAccount/ExternalLDAP_1000.xml
		const t = await soap.getAccountAuthToken(account1Name);
		const res = await soap.makeSOAPEnvelopeAccount(
			'<NoOpRequest xmlns="urn:zimbraMail"/>', t
		);
